<div>
    <div class="mb-8">
        <h2 class="text-3xl font-serif font-bold text-[#3d4d3e] mb-4">Filtrar por:</h2>
    ...
    <h2 class="text-3xl font-serif font-bold text-[#3d4d3e] mb-4">Resumen</h2>
    <div class="flex flex-wrap gap-6 mb-10">
        <div class="bg-white rounded-xl shadow px-8 py-6 flex flex-col items-center justify-center min-w-[220px]">
            <div class="flex items-center gap-3">
                <div class="bg-[#fdf3e7] p-3 rounded-xl">
                    <span class="text-3xl text-yellow-600">🛍️</span>
                </div>
                <span class="text-4xl font-bold text-[#e69b55]">{{ $totalPendientes }}</span>
            </div>
            <p class="text-base font-medium mt-3 text-gray-800">Pedidos pendientes</p>
        </div>

        <div class="bg-white rounded-xl shadow px-8 py-6 flex flex-col items-center justify-center min-w-[220px]">
            <div class="flex items-center gap-3">
                <div class="bg-[#fbece1] p-3 rounded-xl">
                    <span class="text-3xl text-orange-600">🛍️</span>
                </div>
                <span class="text-4xl font-bold text-[#f08535]">{{ $totalCompletados }}</span>
            </div>
            <p class="text-base font-medium mt-3 text-gray-800">Pedidos completados</p>
        </div>
    </div>

    <h2 class="text-3xl font-serif font-bold text-[#3d4d3e] mb-4">Pedidos</h2>
        ...
        @forelse ($pedidos as $pedido)
            <div class="bg-white rounded-xl shadow p-6 relative">
                <div class="flex justify-between items-start">
                    <div>
                        <h3 class="text-2xl font-bold text-[#3d4d3e]">nº {{ $pedido->id }}</h3>
                        <p class="text-sm font-medium text-gray-800 mt-1">
                            {{ \Carbon\Carbon::parse($pedido->fecha)->format('d/m/Y') }} - {{ $pedido->hora }}</p>
                        <p class="text-sm text-gray-600 mt-1">{{ $pedido->cantidad_productos }} productos</p>
                    </div>

                    @if ($pedido->estado === 'pendiente')
                        <span
                            class="px-4 py-1 bg-[#fdf3e7] text-[#e69b55] text-xs font-bold rounded-lg shadow-sm">Pendiente</span>
                    @else
                        <span
                            class="px-4 py-1 bg-[#fbece1] text-[#f08535] text-xs font-bold rounded-lg shadow-sm">Completado</span>
                    @endif
                </div>
                <div class="mt-8 flex justify-between items-center">
                    <p class="text-xl font-bold text-[#3d4d3e]">Total: <span
                            class="text-[#4a5d4e]">{{ number_format($pedido->total, 2) }}€</span></p>

                    <button wire:click="abrirModal({{ $pedido->id }})"
                        class="bg-[#4a5d4e] hover:bg-[#3d4d3e] text-white px-5 py-2 rounded-lg text-sm font-bold shadow-md transition">
                        Ver detalles
                    </button>
                </div>
            </div>
